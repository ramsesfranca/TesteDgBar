...
namespace MinhaAplicacao.Infraestrutura.Repositorios
{
        ...
        where TEntidade : EntidadeBase<TId>
    {
        ...
        public virtual IQueryable<TEntidade> SelecionarPorId(TId id, params string[] navegacao)
        {
            return this.Incluir(navegacao).Where(e => e.Id.Equals(id)).AsNoTracking();
        }

        public virtual IQueryable<TEntidade> SelecionarTodos(params string[] navegacao)
        {
            return this.Incluir(navegacao).AsNoTracking();
        }

        public virtual IQueryable<TEntidade> SelecionarPor(Expression<Func<TEntidade, bool>> predicado, params string[] navegacao)
        {
            return this.Incluir(navegacao).Where(predicado).AsNoTracking();
        }

        private IQueryable<TEntidade> Incluir(params string[] navegacao)
        {
            return navegacao.Aggregate(this.Entidade as IQueryable<TEntidade>, (current, include) => current.Include(include));
        }
