...
namespace MinhaAplicacao.Negocio.Services
{
        ...
        where TRepositorioBase : IRepositorioBase<TId, TEntidade>
    {
        ...
        public virtual async Task<TEntidade> SelecionarPorId(TId id, params string[] navegacao)
        {
            return await this._repositorio.SelecionarPorId(id, navegacao).FirstOrDefaultAsync();
        }

        public virtual async Task<List<TEntidade>> SelecionarTodos(params string[] navegacao)
        {
            return await this._repositorio.SelecionarTodos(navegacao).ToListAsync();
        }

        public virtual async Task<List<TEntidade>> SelecionarPor(Expression<Func<TEntidade, bool>> predicado, params string[] navegacao)
        {
            return await this._repositorio.SelecionarPor(predicado, navegacao).ToListAsync();
        }
