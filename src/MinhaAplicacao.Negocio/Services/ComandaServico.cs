...
namespace MinhaAplicacao.Negocio.Services
{
    public class ComandaServico : ServicoBase<int, Comanda, IComandaRepositorio>, IComandaServico
    {
        ...
        {
            var comanda = await this.SelecionarPorId(id, "Pedidos");
        ...
        {
            var comandas = await this.SelecionarPorId(id, "Pedidos.Cardapio");

            var teste = comandas.Pedidos.GroupBy(p => p.Cardapio.Nome).ToList();
